...
function IMGREG_V2(yourpath, Grid_488_raw, Grid_647_reg, use_fixed_detection)
        ...
        for i = 1:length(ContentInFold)
            if strfind(ContentInFold(i).name, 'STAR') >= 1 & what_a_day_to_code == 0
                ...
                if zz == 0
                    ...
                    figure(2); imshow(double(I_647)./double(imgaussfilt(I_488,1)), [0 5]); title('647/488 ratio'); hold on; set(gca,'xticklabel',{[]}); set(gca,'yticklabel',{[]})
                    m=input('Do you want to continue, Y/N:','s');
